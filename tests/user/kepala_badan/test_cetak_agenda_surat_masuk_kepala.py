...
from pages.cetak_agenda_surat_masuk import CetakanAgendaMasuk


@pytest.fixture
def agenda_masuk_page(login_as, base_url, driver, kepala_badan_creds):
    username, password = kepala_badan_creds
    login_as(username, password)
    menu = MenuNavbar(driver, base_url)
    menu.open_agenda_surat_masuk()
    return CetakanAgendaMasuk(driver, base_url)


@pytest.fixture(scope="session")
def agenda_masuk_data():
    return {
        "tgl_start_ada_data": "12/02/2026",
        "tgl_end_ada_data": "12/03/2026",
        "tgl_start_tidak_ada_data": "20/03/2020",
        "tgl_end_tidak_ada_data": "30/03/2020"
    }


@pytest.mark.kepala_badan
@pytest.mark.cetak_agenda
class TestCetakAgendaMasuk:
    def test_dengan_data(self, agenda_masuk_page, agenda_masuk_data):
        page = agenda_masuk_page
        data = agenda_masuk_data
    ...
    def test_tanpa_data(self, agenda_masuk_page, agenda_masuk_data):
        page = agenda_masuk_page
        data = agenda_masuk_data
